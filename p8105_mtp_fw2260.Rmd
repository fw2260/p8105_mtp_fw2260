...
The `dogweights` dataset contains the weights of three dogs, Gagne, Simone, and Raisin, in pounds, along with some occasional notes, from 2018 to 2020. The dogs were weighed every few days and a standard weight was weighed occasionally as well to ensure accuracy. This report will explain the process of cleaning this dataset and performing some exploratory analyses and visualizations on it.
...
The cleaned `dogweights` dataset is `r nrow(dogweights_df)` rows and `r ncol(dogweights_df)` columns. There are `r nrow(distinct(dogweights_df, date))` unique dates in the dataset. 
